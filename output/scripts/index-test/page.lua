...
--	self.shapegroup = shapegroup:create()
	...
	self.clearshape:setcolor(0.5, 0, 0, 1)
...
--	input.handlers[2] = self.gifield
	...
	window:setshape(self.clearshape)
...
function page:handle_pointdown(message, point, x, y)
--	log(message, point, x, y)
	x = x / info.width
	y = y / info.height
	self.clearshape:setcolor(x, y, 0, 1)
end

function page:handle_pointmove(message, point, x, y)
--	log(message, point, x, y)
	x = x / info.width
	y = y / info.height
	self.clearshape:setcolor(x, y, 0, 1)
...
function page:handle_pointup(message, point)
--	log(message, point)
end
