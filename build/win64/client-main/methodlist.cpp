...
static bool window_window_setshape(::window::Window* self, ::graphics::Shape* nv) NOEXCEPT
{
	try
	{
		self->setshape(nv);
...
static bool window_window_popevent(::window::Window* self, ::window::Event* event, bool* result) NOEXCEPT
{
	try
	{
		*result = self->popevent(event);
		return true;
	}
	catch( ::std::exception const& e )
	{
